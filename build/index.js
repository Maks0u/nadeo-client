...
// src/NadeoServices.js
var NadeoServices = class {
  constructor() {
  }
  static async get(token, path, params = {}) {
    const url2 = new URL(path, NadeoServices.BASEURL);
    url2.search = new URLSearchParams(Object.entries(params)).toString();
    return UbiServices.AXIOS.get(url2.href, {
      headers: {
...
NadeoServices.BASEURL = process.env.NADEO_CORE_BASE || "";

// src/UbiServices.js
var UbiServices = class {
  constructor() {
  }
  ...
  async requestTicket() {
    const url2 = new URL("v3/profiles/sessions", UbiServices.UBI_BASE);
    const response = await UbiServices.AXIOS.post(url2.href, {}, {
      headers: {
  ...
  async requestTokens(ticket, audience) {
    const url2 = new URL("v2/authentication/token/ubiservices", NadeoServices.BASEURL);
    const response = await UbiServices.AXIOS.post(url2.href, { audience }, {
      headers: {
  ...
  async refreshTokens(refreshToken) {
    const url2 = new URL("v2/authentication/token/refresh", NadeoServices.BASEURL);
    const response = await UbiServices.AXIOS.post(url2.href, {}, {
      headers: {
...
UbiServices.AXIOS = axios_default.create({
  headers: {
...
// src/NadeoLiveServices.js
var NadeoLiveServices = class {
  constructor() {
  }
  static async get(token, path, params = {}) {
    const url2 = new URL(path, NadeoLiveServices.BASEURL);
    url2.search = new URLSearchParams(Object.entries(params)).toString();
    return UbiServices.AXIOS.get(url2.href, {
      headers: {
        Authorization: `nadeo_v1 t=${token}`
  ...
  static async getClubMembers(token, clubId, length, offset) {
    return (await this.get(token, `api/token/club/${clubId}/member`, { length, offset })).data;
  }
  static async getCampaigns(token, name = "", length, offset) {
    return (await this.get(token, `api/token/club/campaign`, { length, offset, name })).data;
  }
...
var Token = class {
  ...
  isExpired() {
    return Date.now() > this.decode().exp * 1e3;
  }
  getUserName() {
...
// src/Client.js
var Client = class extends UbiServices {
  constructor() {
    super();
...
function formatTime(time) {
  const minutes = `${Math.floor(time / 6e4)}`.padStart(2, "0");
  const seconds = `${Math.floor(time / 1e3 % 60)}`.padStart(2, "0");
  const millisec = `${Math.round((time / 1e3 - Math.floor(time / 1e3)) * 1e3)}`.padEnd(3, "0");
  return `${minutes}:${seconds}.${millisec}`;
}

// bundle.js
var bundle_default = Client;
